...
- 1.1: In `crates/engine/src/signals/mod.rs`, define:
  ```rust
  ...
  ```
- 1.2: Implement `SignalPipeline::new(obi_config, vpin_config) -> Self` constructor
- 1.3: Implement `SignalPipeline::update_all(&mut self, book: &OrderBook, trade: Option<&MarketEvent>, clock: &dyn Clock)`:
  - Calls `update()` on each signal in sequence
  - Returns tuple or struct of all signal results
- 1.4: Implement `SignalPipeline::all_valid(&self) -> bool`:
  - Returns `self.obi.is_valid() && self.vpin.is_valid() && self.microprice.is_valid()`
- 1.5: Implement `SignalPipeline::reset(&mut self)`:
  - Calls `reset()` on all signals
- 1.6: Implement `SignalPipeline::snapshot(&self) -> PipelineSnapshot`:
  - Captures snapshot from all signals

### Task 2: Implement composite evaluation (AC: weighted scoring, edge conversion, decision_id)
- 2.1: Create `crates/engine/src/signals/composite.rs`
- 2.2: Define `CompositeConfig` struct: `{ obi_weight: f64, vpin_weight: f64, microprice_weight: f64, historical_edge_per_unit: f64 }`
- 2.3: Define `TradeDecision` struct:
  ```rust
  ...
  ```
- 2.4: Define `DecisionReason` enum: `Trade`, `NoTradeSignalInvalid(String)`, `NoTradeEdgeBelowThreshold`, `NoTradeFeeGateBlocked`, `NoTradeNotAtLevel`
- 2.5: Implement `CompositeEvaluator` struct with fields:
  - `config: CompositeConfig`
  - `decision_counter: u64` — monotonic counter for decision_id generation
- 2.6: Implement `CompositeEvaluator::evaluate(&mut self, pipeline: &SignalPipeline, fee_gate: &FeeGate, clock: &dyn Clock) -> TradeDecision`:
...
- 3.1: Create `crates/engine/src/risk/fee_gate.rs`
- 3.2: Create `crates/engine/src/risk/mod.rs` with `pub mod fee_gate;`
- 3.3: Update `crates/engine/src/lib.rs` to declare `pub mod risk`
- 3.4: Define `FeeGate` struct matching architecture:
  ```rust
  pub struct FeeGate {
  ...
  ```
- 3.5: Implement `FeeGate::total_round_trip_cost(&self) -> FixedPrice`:
...
- 3.6: Implement `FeeGate::permits_trade(&self, expected_edge: FixedPrice, clock: &dyn Clock) -> Result<bool, FeeGateReason>`:
...
- 3.7: Implement `FeeGate::from_config(config: &FeeConfig) -> Self`:
  - Load from `core/src/config/fees.rs` FeeConfig struct
- 3.8: Define `FeeGateReason` enum: `Permitted`, `EdgeBelowThreshold { edge: FixedPrice, threshold: FixedPrice }`, `StaleSchedule { days: u64 }`

### Task 4: Staleness safety exception (AC: never blocks flattening/safety)
- 4.1: Implement `FeeGate::permits_flatten(&self) -> bool`: always returns `true` — staleness gate never applies to safety/flatten orders
- 4.2: Document in code comments: "Staleness gate applies to trade evaluation only. Flattening and safety orders are never gated."
- 4.3: The caller (event loop / order manager) must use `permits_flatten()` for safety orders, `permits_trade()` for new trade evaluation

### Task 5: Decision logging with causality tracing (AC: decision_id in all logs)
- 5.1: All log lines in `evaluate()` include `decision_id` field for structured tracing
- 5.2: Trade decision log: `tracing::info!(decision_id = %id, direction = ?dir, edge = %edge, score = %score, obi = %obi, vpin = %vpin, microprice = %mp, "trade decision")`
- 5.3: No-trade decision log: `tracing::info!(decision_id = %id, reason = ?reason, "no-trade decision")`
- 5.4: Fee gate block log: includes cost breakdown and edge vs threshold

### Task 6: Write unit tests (AC: all evaluation paths, fee computation, staleness)
- 6.1: Test: SignalPipeline construction with concrete types, all_valid() when all signals valid
- 6.2: Test: all_valid() false when any one signal is invalid
- 6.3: Test: composite score computation with known weights and signal values
- 6.4: Test: direction derived correctly from positive/negative composite score
- 6.5: Test: expected edge conversion to FixedPrice via banker's rounding
- 6.6: Test: decision_id is unique and incrementing across evaluations
- 6.7: Test: FeeGate total_round_trip_cost computation:
  - exchange=2qt, commission=1qt, api=1qt, slippage=2qt per side
  - total = 2*(2+1+1) + 2 = 10 quarter-ticks
- 6.8: Test: FeeGate permits_trade true when edge > multiple * cost
- 6.9: Test: FeeGate permits_trade false when edge <= multiple * cost
- 6.10: Test: FeeGate staleness warning at 31 days (permits, but warns)
- 6.11: Test: FeeGate staleness block at 61 days (blocks trade evaluation)
- 6.12: Test: FeeGate permits_flatten always true even when stale
- 6.13: Test: full evaluate() path — all signals valid, edge sufficient = Trade decision
- 6.14: Test: full evaluate() path — one signal invalid = NoTradeSignalInvalid
- 6.15: Test: full evaluate() path — edge below threshold = NoTradeEdgeBelowThreshold
- 6.16: All tests use `testkit::SimClock`
